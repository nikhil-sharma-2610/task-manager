"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var cors_1 = require("cors");
var Task_js_1 = require("./models/Task.js");
var db_js_1 = require("./db.js");
var app = (0, express_1.default)();
var port = process.env.PORT || 6001;
// Middleware
app.use((0, cors_1.default)());
app.use(express_1.default.json());
// Sync the database with Sequelize
db_js_1.default.sync({ force: true }).then(function () {
    console.log("Database synced successfully!");
}).catch(function (error) {
    console.error("Error syncing database:", error);
});
// POST /tasks - Create a new task
app.post("/tasks", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, title, description, dueDate, priority, status, newTask, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, title = _a.title, description = _a.description, dueDate = _a.dueDate, priority = _a.priority, status = _a.status;
                _b.label = 1;
            case 1:
                _b.trys.push([1, 3, , 4]);
                return [4 /*yield*/, Task_js_1.default.create({
                        title: title,
                        description: description,
                        dueDate: dueDate,
                        priority: priority,
                        status: status
                    })];
            case 2:
                newTask = _b.sent();
                // Send back the newly created task
                res.status(201).json(newTask);
                return [3 /*break*/, 4];
            case 3:
                error_1 = _b.sent();
                console.error("Error creating task:", error_1);
                res.status(500).json({ message: "Error creating task" });
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); });
// PUT /tasks/:id - Update a task
app.put("/tasks/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, _a, title, description, dueDate, priority, status, task, error_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                id = req.params.id;
                _a = req.body, title = _a.title, description = _a.description, dueDate = _a.dueDate, priority = _a.priority, status = _a.status;
                _b.label = 1;
            case 1:
                _b.trys.push([1, 4, , 5]);
                return [4 /*yield*/, Task_js_1.default.findByPk(id)];
            case 2:
                task = _b.sent();
                if (!task) {
                    res.status(404).json({ message: "Task not found" });
                    return [2 /*return*/];
                }
                // Type-safe updates
                if (title !== undefined)
                    task.title = title;
                if (description !== undefined)
                    task.description = description;
                if (dueDate !== undefined)
                    task.dueDate = dueDate;
                if (priority !== undefined)
                    task.priority = priority;
                if (status !== undefined)
                    task.status = status;
                return [4 /*yield*/, task.save()];
            case 3:
                _b.sent();
                res.json(task);
                return [3 /*break*/, 5];
            case 4:
                error_2 = _b.sent();
                console.error("Error updating task:", error_2);
                res.status(500).json({ message: "Error updating task" });
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); });
// DELETE /tasks/:id - Delete a task
app.delete("/tasks/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, task, error_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params.id;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 4, , 5]);
                return [4 /*yield*/, Task_js_1.default.findByPk(id)];
            case 2:
                task = _a.sent();
                if (!task) {
                    res.status(404).json({ message: "Task not found" });
                    return [2 /*return*/];
                }
                return [4 /*yield*/, task.destroy()];
            case 3:
                _a.sent();
                res.status(204).end();
                return [3 /*break*/, 5];
            case 4:
                error_3 = _a.sent();
                console.error("Error deleting task:", error_3);
                res.status(500).json({ message: "Error deleting task" });
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); });
// Start the server
app.listen(port, function () {
    console.log("Server running at http://localhost:".concat(port));
});
