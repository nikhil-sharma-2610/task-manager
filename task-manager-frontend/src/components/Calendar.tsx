...
interface CalendarViewProps {
  tasks: Task[]; // Add tasks as a prop
...
const CalendarView = ({ onTaskClick }: CalendarViewProps) => {
  ...
  const events = tasks.map((task) => {
    ...
    return {
      id: task.id,
      title: task.title,
      date: new Date(task.dueDate).toISOString().split("T")[0], // Format to YYYY-MM-DD
      backgroundColor,
    ...
    <div className="bg-white p-4 rounded-lg shadow">
      <FullCalendar
        ...
        eventClick={(info) => {
          ...
          if (task) {
            onTaskClick(task); // Trigger the onTaskClick callback when a task is clicked
          }
        }}
        height="auto"
        headerToolbar={{
